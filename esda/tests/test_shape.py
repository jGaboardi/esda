...
test_geom_translated = shapely.from_wkt(
    [
        "POLYGON (("
        "-3.1823503126754247 0.085191513232644, "
        "-3.2545854200972997 0.271135116748269, "
        "-3.2472001661910497 0.296769882373269, "
        "-3.2779008497847997 0.3333146821779565, "
        "-3.286461030448862 0.4668824922365502, "
        "-3.312919770683237 0.4887788545412377, "
        "-3.308738862480112 0.5528352510256127, "
        "-3.271751557792612 0.6005037080568627, "
        "-3.2749711622847997 0.6791780244631127, "
        "-3.301475678886362 0.7266938936037377, "
        "-3.3279496779097997 0.7266252290529565, "
        "-3.3439103712691747 0.8217256318849877, "
        "-3.385307465995737 0.9057634126467065, "
        "-3.385490571464487 1.0868776094240502, "
        "-3.4014665236129247 1.1563432466310815, "
        "-3.3219682325972997 1.1584108125490502, "
        "-3.3168107618941747 1.328538681201394, "
        "-3.2181779493941747 1.3360688936037377, "
        "-3.2150346388472997 1.4000871431154565, "
        "-3.1089555372847997 1.4057023774904565, "
        "-3.105888520683237 1.9232576143068627, "
        "-2.702140962089487 1.9063203584474877, "
        "-2.7046891798629247 0.8271501313967065, "
        "-2.749656831230112 0.7665956270021752, "
        "-2.671684419120737 0.5808656465334252, "
        "-2.8658219923629247 0.3313920747560815, "
        "-2.9108354200972997 0.1093156587404565, "
        "-3.1823503126754247 0.085191513232644"
        "))"
    ]
)[0]
...
# Create grid for testing of moment_of_inertia_regions
tmp_geoms = []
for i in range(8):
    for j in range(8):
...
def test_moment_of_inertia():
    observed = esda.shape.moment_of_inertia(testbench.geometry)
...
def test_moment_of_inertia_normalized():
    observed = esda.shape.moment_of_inertia(testbench.geometry, normalize=True)
    testing.assert_allclose(
        observed,
        [0.58666692, 0.78130608, 0.30954837, 0.69905169, 0.175372],
        atol=ATOL,
    )


def test_moment_of_inertia_about_origin():
    observed = esda.shape.moment_of_inertia(testbench.geometry, ref_pt=(0, 0))
    testing.assert_allclose(
        observed,
        [9.53818256, 2.25, 5.52604167, 5.24375414, 87.06458556],
        atol=ATOL,
    )
...
def test_mir_with_regions():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(test_region_8x8, regions=regions)
    )
    testing.assert_allclose(
        observed,
        [7.66666667, 205.22222222, 90.66666667, 96.0],
        atol=ATOL,
    )
...
def test_mir_with_region_col():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(test_region_8x8, regions="region_name")
    )
    testing.assert_allclose(
        observed,
        [7.66666667, 205.22222222, 90.66666667, 96.0],
...
def test_mir_with_regions_normalized():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(
            test_region_8x8, normalize=True, regions=regions
        )
    )
    testing.assert_allclose(
        observed,
...
def test_mir_with_weights():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(test_region_8x8, weights=masses)
    )
    testing.assert_allclose(
        observed,
        [
...
def test_mir_with_weight_col():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(test_region_8x8, weights="mass")
    )
    testing.assert_allclose(
        observed,
        [
...
def test_mir_with_weights_normalized():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(
            test_region_8x8, normalize=True, weights=masses
        )
    )
    testing.assert_allclose(
        observed,
...
def test_mir_with_regions_and_weights():
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(
            test_region_8x8, regions=regions, weights=masses
        )
    )
    testing.assert_allclose(
        observed,
...
def test_mir_with_regions_and_ref_pts():
    ...
    observed = numpy.asarray(
        esda.shape.moment_of_inertia_regions(
            test_region_8x8, ref_pt=ref_pt, regions=regions
        )
    )
    testing.assert_allclose(
        observed,
        [14.5, 520.0, 234.66666667, 144.0],
        atol=ATOL,
    )

...
class Lee_Tester(unittest.TestCase):
    ...
    def test_global(self):
        ...
        known = numpy.array([[0.30136527, -0.23625603], [-0.23625603, 0.53512008]])
        ...
        first_rep = numpy.array([[0.22803705, -0.08053692], [-0.08053692, 0.18897318]])

        second_rep = numpy.array([[0.14179274, -0.06962692], [-0.06962692, 0.13688337]])
        ...
        known_significance = numpy.array([[0.125, 0.026], [0.026, 0.001]])
        numpy.testing.assert_allclose(
    ...
    def test_local(self):
        numpy.random.seed(2478879)
        result = lee.Spatial_Pearson_Local(connectivity=self.w.sparse).fit(
            self.x, self.y
        )
        known_locals = numpy.array(
            [
                0.10246023,
                -0.24169198,
                -0.1308714,
        ...
        numpy.testing.assert_allclose(
            known_locals, result.associations_, rtol=RTOL, atol=ATOL
        )
        significances = numpy.array(
            [
                0.154,
                0.291,
                0.358,
